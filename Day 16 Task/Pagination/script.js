...
<h1 id="title">Pagination</h1>
    <p id="description" class="">Pagination Example</p>
    ...
    <div class="pagi-container d-flex justify-content-center" id="buttons">
...
let itemPerPage = 10;
function pagination(length, itemPerPage) {
  let container = document.getElementsByClassName("pagi-container")[0];
  let pages = length / itemPerPage;
  if (pages % 1 != 0) {
    pages = parseInt(pages);
    pages++;
  }
  container.innerHTML += `<button class="previous page-link" href="#">Pervious</button> `;
  for (let i = 1; i <= pages; i++) {
    container.innerHTML += `<button class="page-link" href="#">${i}</button>`;
  }
  container.innerHTML += `<button class="next page-link" href="#">Next</button>`;
}
...
document.querySelectorAll(".page-link").forEach((item) => {
    ...
    let pagedlength = itemPerPage;
    console.log((pageNo*pagedlength-9),pageNo*pagedlength)
    let data = users.slice((pageNo*pagedlength-9)-1,pageNo*pagedlength)
    // data.forEach((user) => {
...
let data = users.slice((1*itemPerPage-9)-1,1*itemPerPage)
    data.forEach((user) => {
        document.getElementById("data").innerHTML += `
        <tr>
        <td>${user.name}</td>
